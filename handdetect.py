...
def get_hands(image):
    ...
    cv.Threshold(hue, hue, 18, 255, cv.CV_THRESH_TOZERO) #set to 0 if <= 10, otherwise leave as is
    ...
    cv.Threshold(sat, sat, 74, 255, cv.CV_THRESH_TOZERO) #set to 0 if <= 64, otherwise leave as is
    cv.EqualizeHist(sat, sat)

    cv.Threshold(sat, sat, 74, 255, cv.CV_THRESH_BINARY) #set to 0 if <= 64, otherwise 255
